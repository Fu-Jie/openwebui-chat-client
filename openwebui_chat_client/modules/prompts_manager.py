...
class PromptsManager:
    ...
    def get_prompts_list(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            logger.info(
                f"Successfully retrieved prompts list with {len(prompts_list)} items."
            )
            return prompts_list
        except requests.exceptions.RequestException as e:
    ...
    def create_prompt(
        ...
        try:
            ...
            logger.info(
                f"Successfully created prompt '{command}' with title '{title}'."
            )
            return prompt
        except requests.exceptions.RequestException as e:
    ...
    def update_prompt_by_command(
        ...
        payload = {
            ...
            "content": (
                content if content is not None else current_prompt.get("content", "")
            ),
        }
    ...
    def replace_prompt_by_command(
        ...
        if old_command != new_command:
            ...
            if existing_new:
                logger.error(
                    f"Cannot replace prompt: new command '{new_command}' already exists."
                )
                return None
        ...
        if not delete_success:
            logger.error(
                f"Failed to delete old prompt '{old_command}'. Replacement aborted."
            )
            return None
        ...
        if new_prompt:
            logger.info(
                f"Successfully replaced prompt '{old_command}' with '{new_command}'."
            )
            return new_prompt
        else:
            logger.error(
                f"Failed to create new prompt '{new_command}'. Original prompt was deleted!"
            )
            ...
            restored = self.create_prompt(
                ...
                old_prompt.get(
                    "content", "Original content was lost during replacement failure"
                ),
            ...
            if restored:
                ...
                logger.error(
                    f"Failed to restore original prompt '{old_command}'. Data may be lost!"
                )
            return None
    ...
    def delete_prompt_by_command(self, command: str) -> bool:
        ...
        try:
            ...
            if e.response.status_code == 404:
                logger.warning(
                    f"Prompt with command '{command}' not found for deletion."
                )
                return False
    ...
    def batch_create_prompts(
        ...
        results = {"success": [], "failed": [], "total": len(prompts_data)}
        ...
        for prompt_data in prompts_data:
            try:
                ...
                if not all([command, title, content]):
                    logger.error(
                        f"Missing required fields in prompt data: {prompt_data}"
                    )
                ...
                created_prompt = self.create_prompt(
                ...
                if created_prompt:
                    ...
                    results["failed"].append(
                        {"data": prompt_data, "error": "Failed to create prompt"}
                    )

            except Exception as e:
                logger.error(f"Error creating prompt from data {prompt_data}: {e}")
                results["failed"].append({"data": prompt_data, "error": str(e)})

                if not continue_on_error:
                    break

        logger.info(
            f"Batch creation completed. Success: {len(results['success'])}, Failed: {len(results['failed'])}"
        )
        return results
    ...
    def batch_delete_prompts(
        ...
        results = {"success": [], "failed": [], "total": len(commands)}
        ...
        for command in commands:
            try:
                ...
                if success:
                    ...
                    results["failed"].append(
                        {"command": command, "error": "Failed to delete prompt"}
                    )

            except Exception as e:
                logger.error(f"Error deleting prompt '{command}': {e}")
                results["failed"].append({"command": command, "error": str(e)})

                if not continue_on_error:
                    break

        logger.info(
            f"Batch deletion completed. Success: {len(results['success'])}, Failed: {len(results['failed'])}"
        )
        return results
