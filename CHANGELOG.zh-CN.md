...
- **增强任务处理提示词**：优化了任务处理系统提示词，新增"关键发现"(Key Findings)累积机制，确保工具调用结果在整个问题解决过程中持续注入到AI的上下文中。AI现在会被指示维护并引用所有工具调用积累的知识。

- **决策模型支持**：为 `process_task` 和 `stream_process_task` 方法新增 `decision_model_id` 参数。当AI提出多个解决方案选项时，可以使用单独的决策模型自动分析并选择最佳选项，无需用户干预。

- **选项检测**：新增 `_detect_options_in_response` 辅助方法，用于解析和识别AI在响应中提出的多个编号选项。

- **自动选项选择**：新增 `_get_decision_from_model` 辅助方法，使用决策模型基于上下文和原始任务需求分析选项并做出明智选择。

- **新的流事件类型**：为 `stream_process_task` 新增 `decision` 事件类型，在决策模型选择选项时提供实时可见性。

### 变更

- **改进系统提示词**：任务处理提示词现在强调维护一个"关键发现"部分，累积所有工具调用的结果，确保这些洞察在整个任务解决过程中可用并被引用。

## [0.1.23] - 2025-12-23

### 新增

- **异步客户端**：引入基于 `httpx` 的 `AsyncOpenWebUIClient`，用于高性能异步应用。
- **用户管理**：新增 `UserManager` 与 `AsyncUserManager`，支持用户列表、详情、角色更新与删除。
- **集成冒烟测试**：新增同步流式聊天的环境门控冒烟测试，并在 `examples/README.md` 记录全部集成测试。

### 修复

- **同步流式稳定性**：在流式拼接前跳过 `None` 块并字符串化非字符串块，避免运行时错误。
- **CI 映射**：更新 `.github/test-mapping.yml`，加入 `sync_live_stream` 分类并映射核心同步流式路径。
