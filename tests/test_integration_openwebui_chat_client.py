...
class TestIntegrationOpenWebUIClient(unittest.TestCase):
    ...
    def test_list_models_integration(self):
        ...
        self.assertIsInstance(models, list, "The list_models() call should return a list.")
    ...
    def test_chat_integration(self):
        ...
        if test_model_id not in model_ids:
            if not model_ids:
                self.skipTest("Skipping chat integration test: No models found on the server.")
        ...
        try:
            ...
            if e.response.status_code >= 500:
                self.skipTest(f"Skipping chat test: Server returned a {e.response.status_code} error for model '{test_model_id}'.")
        ...
        self.assertTrue(len(result["response"]) > 0, "The response should not be empty.")
    ...
    def test_model_management_integration(self):
        ...
        if base_model_id not in model_ids:
            if not model_ids:
                self.skipTest("Skipping model management test: No models found on the server to use as a base.")
        ...
        self.assertIsNotNone(created_model, "create_model should return the created model.")
        ...
        self.assertEqual(read_model.get('meta', {}).get('description'), "Initial description.")
        ...
        self.assertIsNotNone(updated_model, "update_model should return the updated model.")
        self.assertEqual(updated_model.get('meta', {}).get('description'), "Updated description.")
        ...
        self.assertIsNone(deleted_model, "get_model should return None for a deleted model.")
