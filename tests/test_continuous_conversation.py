...
class TestContinuousConversation(unittest.TestCase):
    ...
    def test_continuous_chat_single_round(self):
        ...
        with patch.object(self.client._chat_manager, "chat") as mock_chat:
            ...
            self.assertEqual(
                result["conversation_history"][0]["response"], "Test response"
            )
    ...
    def test_continuous_chat_multiple_rounds(self):
        ...
        with patch.object(self.client._chat_manager, "chat") as mock_chat:
            ...
            self.assertEqual(result["conversation_history"][0]["question"], "Hello")
            self.assertEqual(
                result["conversation_history"][0]["response"], "First response"
            )

            # Check second round (using follow-up)
            self.assertEqual(
                result["conversation_history"][1]["question"], "What about this?"
            )
            self.assertEqual(
                result["conversation_history"][1]["response"], "Second response"
            )

            # Check third round (using follow-up)
            self.assertEqual(
                result["conversation_history"][2]["question"], "Another question?"
            )
            self.assertEqual(
                result["conversation_history"][2]["response"], "Third response"
            )
    ...
    def test_continuous_chat_no_follow_ups(self):
        ...
        with patch.object(self.client._chat_manager, "chat") as mock_chat:
            ...
            self.assertEqual(
                result["conversation_history"][1]["question"],
    ...
    def test_continuous_chat_invalid_num_questions(self):
        ...
        result = self.client.continuous_chat(
            initial_question="Hello", num_questions=0, chat_title="Test Continuous Chat"
        )
    ...
    def test_continuous_parallel_chat_success(self):
        ...
        with patch.object(
            self.client._chat_manager, "parallel_chat"
        ) as mock_parallel_chat:
            ...
            self.assertEqual(
                result["conversation_history"][0]["follow_ups"],
