...
class TestFollowUpFeature(unittest.TestCase):
    ...
    def _mock_chat_creation_and_loading(self, chat_id="test_chat_id", title="Test Chat"):
            ...
            if "/api/v1/tasks/config" in url:
                ...
                return response
                 ...
                 response.json.return_value = {
                    ...
                    "chat": {"id": chat_id, "title": title, "models": ["test_model"], 
        ...
        self.mock_session.post.side_effect = lambda *args, **kwargs: mock_create_response if "/api/v1/chats/new" in args[0] else MagicMock()

    @patch('openwebui_chat_client.openwebui_chat_client.OpenWebUIClient._update_remote_chat')
    def test_chat_with_follow_up(self, mock_update_remote_chat):
        ...
        result = self.client.chat(question="Test question", chat_title="Test Chat", enable_follow_up=True)

        # Assertions
        self.assertEqual(mock_update_remote_chat.call_count, 2, "Should update remote chat twice")
        ...
        self.assertTrue(follow_up_call.args[0].endswith("/api/v1/tasks/follow_up/completions"))
        ...
        last_message_id = self.client.chat_object_from_server["chat"]["history"]["currentId"]
        last_message = self.client.chat_object_from_server["chat"]["history"]["messages"][last_message_id]
