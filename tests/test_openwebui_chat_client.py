...
class TestOpenWebUIClient(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(self.client.json_headers["Authorization"], expected_auth_header)
    ...
    def test_list_base_models_success(self, mock_get):
        ...
        self.assertEqual(len(result), 2)
        mock_get.assert_called_once_with(
            f"{self.base_url}/api/models/base",
            headers=self.client.json_headers
        )
    ...
    def test_list_custom_models_success(self, mock_get):
        ...
        self.assertEqual(len(result), 2)
        mock_get.assert_called_once_with(
            f"{self.base_url}/api/v1/models/custom",
            headers=self.client.json_headers
        )
    ...
    def test_create_folder_success(self, mock_post):
        ...
        with patch.object(self.client, 'get_folder_id_by_name', return_value="new-folder-id"):
    ...
    def test_handle_rag_references_files_only(self):
        ...
        with patch.object(self.client, '_upload_file') as mock_upload:
            ...
            self.assertEqual(storage_payload[0]["type"], "application/pdf")  # From file_obj
            self.assertIn("file", storage_payload[0])
            self.assertEqual(storage_payload[0]["id"], "file123")  # Spread from file_obj
            self.assertEqual(storage_payload[0]["name"], "test.pdf")  # Spread from file_obj
    ...
    def test_handle_rag_references_collections_only(self):
        ...
        with patch.object(self.client, 'get_knowledge_base_by_name', return_value=kb_summary):
            with patch.object(self.client, '_get_knowledge_base_details', return_value=kb_details):
                api_payload, storage_payload = self.client._handle_rag_references(
                    rag_files=None,
