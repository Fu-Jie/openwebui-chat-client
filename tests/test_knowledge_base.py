...
class TestOpenWebUIClientKnowledgeBase(unittest.TestCase):
    ...
    @patch('openwebui_chat_client.openwebui_chat_client.requests.Session.post')
    def test_add_file_to_existing_knowledge_base(self, mock_post, mock_upload, mock_get_kb):
    ...
    def test_delete_all_knowledge_bases_success(self, mock_as_completed, mock_executor, mock_get):
    ...
    @patch('openwebui_chat_client.openwebui_chat_client.ThreadPoolExecutor')
    @patch('openwebui_chat_client.openwebui_chat_client.as_completed')
    def test_delete_knowledge_bases_by_keyword_success(self, mock_as_completed, mock_executor, mock_get):
        ...
        successful, failed, names = self.client.delete_knowledge_bases_by_keyword(keyword)
    ...
    @patch('openwebui_chat_client.openwebui_chat_client.ThreadPoolExecutor')
    @patch('openwebui_chat_client.openwebui_chat_client.as_completed')
    def test_create_knowledge_bases_with_files_success(self, mock_as_completed, mock_executor):
        ...
        with patch.object(self.client, 'create_knowledge_base') as mock_create:
            with patch.object(self.client, 'add_file_to_knowledge_base') as mock_add_file:
    ...
    @patch('openwebui_chat_client.openwebui_chat_client.ThreadPoolExecutor')
    @patch('openwebui_chat_client.openwebui_chat_client.as_completed')
    def test_create_knowledge_bases_with_files_partial_failure(self, mock_as_completed, mock_executor):
        """Test batch creation with some failures."""
        kb_files = {
            "Success KB": ["/tmp/file1.pdf"],
            "Fail KB": ["/tmp/file2.pdf"]
        }
    ...
    @patch.object(OpenWebUIClient, '_upload_file')
    @patch('openwebui_chat_client.openwebui_chat_client.requests.Session.post')
    def test_add_file_to_knowledge_base_comprehensive_flow(self, mock_post, mock_upload, mock_create, mock_get_kb):
    ...
    def test_handle_rag_references_mixed_scenario(self):
        ...
        with patch.object(self.client, '_upload_file') as mock_upload:
            ...
            with patch.object(self.client, 'get_knowledge_base_by_name') as mock_get_kb:
                with patch.object(self.client, '_get_knowledge_base_details') as mock_get_kb_details:
