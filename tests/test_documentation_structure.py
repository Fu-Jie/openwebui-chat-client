...
class TestDocumentationStructure(unittest.TestCase):
    ...
    def test_readme_files_exist(self):
        ...
        self.assertTrue(
            self.copilot_instructions.exists(), "Copilot instructions not found"
        )

    def test_api_reference_structure_english(self):
        """Test that English README has proper API Reference structure."""
        with open(self.readme_en, encoding="utf-8") as f:
            content = f.read()
    ...
    def test_api_reference_structure_chinese(self):
        ...
        with open(self.readme_zh, encoding="utf-8") as f:
            content = f.read()
    ...
    def test_table_structure_consistency(self):
        ...
        for file_path, language in [
            ...
            api_section_match = re.search(
                api_section_pattern, content, re.MULTILINE | re.DOTALL
            )
            if api_section_match:
                ...
                self.assertGreater(
                    table_headers_count,
                    0,
                    f"{language}: No properly structured tables found",
                )
            else:
    ...
    def test_copilot_instructions_enhanced(self):
        ...
        with open(self.copilot_instructions, encoding="utf-8") as f:
            content = f.read()
        ...
        for section in expected_sections:
            self.assertIn(
                section, content, f"Missing copilot instruction section: {section}"
            )
    ...
    def test_return_value_examples_present(self):
        ...
        for file_path, language in [
            ...
            self.assertGreater(
                len(code_blocks), 0, f"{language}: No code examples found"
            )

            # Check for specific return value patterns
            expected_patterns = ['"response":', '"chat_id":', '"message_id":']
            ...
            self.assertGreater(
                found_patterns, 0, f"{language}: No return value examples found"
            )
